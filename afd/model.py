...
class Model(ABC):
    ...
    def __init__(self, seed, lr, optimizer=None):
        ...
        with self.graph.as_default():
            ...
            self.features, self.labels, self.train_op, self.eval_metric_ops, self.loss, self.act_1 = self.create_model()
    ...
    def train(self, data, num_epochs=1, batch_size=10, actorgrad=None):
        """
        ...
        for _ in range(num_epochs):
            act = self.run_epoch(data, batch_size, actorgrad)
            if actorgrad == "act":
                for i in range(0, len(act)):
                    activations.append(act[i])

        if actorgrad == "act":    
            finalact = np.mean(np.array(activations), axis=0)

            
        update = self.get_params()
        comp = num_epochs * (len(data['y'])//batch_size) * batch_size * self.flops
        return comp, update, finalact

    def run_epoch(self, data, batch_size, actorgrad=None):
        acts = []
        for batched_x, batched_y in batch_data(data, batch_size, seed=self.seed):
            ...
            if actorgrad == "act":
                with self.graph.as_default():
                    act_1, _ = self.sess.run([self.act_1, self.train_op],
                        feed_dict={
                            self.features: input_data,
                            self.labels: target_data
                        })
                for i in range(0, len(act_1)):
                    acts.append(act_1[i])
            else:
                with self.graph.as_default():
