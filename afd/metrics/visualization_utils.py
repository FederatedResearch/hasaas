...
def load_data(stat_metrics_file='metrics_stat.csv', sys_metrics_file='metrics_sys.csv'):
...
def plot_accuracy_vs_round_number_per_client(
    ...
    labels = ['%s, %d' % (row[CLIENT_ID_KEY], row[NUM_SAMPLES_KEY])
...
def plot_client_computations_vs_round_number(
    ...
    comp_matrix = []
    matrix_keys = [c for c in clients]
