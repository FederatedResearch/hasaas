...
import numpy as np
from flask import Response, current_app, render_template, request, send_file, send_from_directory
...
from ...core.exceptions import InvalidRequestKeyError, ModelNotFoundError, PyGridError,CycleNotFoundError
from ...events.model_centric.fl_events import (
    assign_worker_id,
    cycle_request,
    save_fcm_device_registration_token,
    update_worker_online_status,
    report,
    requires_speed_test,
    process_report_stats,
)
...
from ...model_centric.metrics import metrics_manager
...
@model_centric_routes.route("/save-fcm-token", methods=["POST"])
def save_fcm_token():
    """Save firebase token against each worker."""
    status_code = None
    try:
        body = json.loads(request.data)
        response_body = save_fcm_device_registration_token({MSG_FIELD.DATA: body}, None, cycle_manager)
    except (PyGridError, json.decoder.JSONDecodeError) as e:
        status_code = 400  # Bad Request
        response_body[RESPONSE_MSG.ERROR] = str(e)
    except Exception as e:
...
@model_centric_routes.route("/send_push_event", methods=["POST"])
def send_push():
    status_code = None
    response_body = {}
    try:
        body = json.loads(request.data)
        response_body = send_push_event({MSG_FIELD.DATA: body}, None)
    except (PyGridError, json.decoder.JSONDecodeError) as e:
        status_code = 400  # Bad Request
        raise SystemExit(0)
        response_body[RESPONSE_MSG.ERROR] = str(e)
...
@model_centric_routes.route("/report_stats", methods=["POST"])
def report_stats():
    response_body = {}
    status_code = None
    ...
    try:
        body = json.loads(request.data)
        response_body = process_report_stats({MSG_FIELD.DATA: body}, None)
    except (PyGridError, json.decoder.JSONDecodeError) as e:
        status_code = 400  # Bad Request
        response_body[RESPONSE_MSG.ERROR] = str(e)
    except Exception as e:
...
def download_model():
    ...
    try:
        ...
        server_config, _ = process_manager.get_configs(id=_cycle.fl_process_id)
        is_pruning_enabled = server_config.get("is_pruning_enabled", True)
        is_force_pruning = server_config.get("is_force_pruning", False)
        seed = server_config.get("seed", True)

        # if client is slow then send the pruned model to the client
        if (_worker.is_slow and is_pruning_enabled) or is_force_pruning:
            logging.info(f"================== serving slow model to worker_id {worker_id}")
            smaller_model = model_manager.prune_model(model_id, seed)
            _last_checkpoint = smaller_model
            # print(model_manager.unserialize_model_params(_last_checkpoint))
            # exit()
        else:
            logging.info(f"================== serving large model to worker_id {worker_id}")
            _last_checkpoint = model_manager.load(model_id=model_id).value

        # unserialized_diff = model_manager.unserialize_model_params(_last_checkpoint)
        # list_model_params = []
        # for param in unserialized_diff:
        #     list_model_params.append(param.data.numpy())
        #
        # np_weights = np.array(list_model_params)
        # np.save(f'testing_download', np_weights)


        return send_file(
            io.BytesIO(_last_checkpoint), mimetype="application/octet-stream"
        )

        # return send_from_directory("/media/aliza16/34a93c51-067c-4a49-8db0-907586ac455c/home/aliza/Mustansar/Softwares/private-repositories/pygrid-federated/apps/node", "all_data_0.json", mimetype="application/json")
...
@model_centric_routes.route("/update-worker-status", methods=["POST"])
def update_worker_status():
    ...
    try:
        ...
        logging.info(f"=================== ${model_name}, ${model_version}")
        verification_result = verify_token(_auth_token, model_name, model_version)
        logging.info(f"=================== ${verification_result}")
        if verification_result["status"] == RESPONSE_MSG.SUCCESS:

            update_worker_online_status({MSG_FIELD.DATA: data})
            # check if requires speed test
            response_body[CYCLE.STATUS] = RESPONSE_MSG.SUCCESS

            
            #response_body = save_fcm_device_registration_token({MSG_FIELD.DATA: body}, None)
...
def check_cycle_expiration():

    app = scheduler.app
    with app.app_context():
        try:
            cycle_manager.check_cycle_expiration()
        except:
            logging.info(f"No active cycle available")

    return str('ok')

# @model_centric_routes.route("/upload_stats_file", methods=["POST"])
# def upload_file():
#    if request.method == 'POST':
#       f = request.files['file']
#       #f.save(f.filename)
#       app = scheduler.app
#       send_from_directory(app.config['UPLOAD_FOLDER'], f.filename)
#       return f.filename

@model_centric_routes.route("/upload_stats", methods=["POST"])
def upload_stats():


    body = json.loads(request.data)
    msg_list = body.get(MSG_FIELD.STATS)
    logging.info(f"request.data {body.get}")
    metrics_manager.store(msg_list)

    resp = {}
    resp[CYCLE.STATUS] = "Stats have been uploaded."

    return {
        MSG_FIELD.TYPE: "model-centric/upload_stats",
        MSG_FIELD.DATA: resp,
    }

@model_centric_routes.route("/training-metrics", methods=["GET"])
def get_training_stats():
    resp = {}
    resp[CYCLE.STATUS] = "Stats have been uploaded."
    resp["stats"] = metrics_manager.get_all_stats()

    response_body = json.dumps({
        MSG_FIELD.TYPE: "model-centric/training-metrics",
        MSG_FIELD.DATA: resp,
    })

    return Response(response_body, status=200, mimetype="application/json")

@model_centric_routes.route("/get_activations", methods=["GET"])
def get_activations():
    resp = {}
    resp[CYCLE.STATUS] = "Stats have been uploaded."
    resp["activations"] = processes.get_activations()

    response_body = json.dumps({
        MSG_FIELD.TYPE: "model-centric/get_activations",
        MSG_FIELD.DATA: resp,
    })
    return Response(response_body, status=200, mimetype="application/json")