...
class ClientModel(Model):
    ...
    def create_model(self):
        if self.type == 'H':
            ...
            out = input_ph
            for _ in range(4):
                name = "conv_batch" + str(_+1) if (_<3) else "conv_last_batch"
                out = tf.layers.conv2d(out, 32, 3, padding='same', name=name)
            ...
            return input_ph, label_ph, minimize_op, eval_metric_ops, tf.math.reduce_mean(loss), None
            ...
            for _ in range(4):
                name = "conv_batch" + str(_+1) if (_<3) else "conv_last_batch"
                mask = self.masks[name+'/kernel:0'][2]
                new_filters = np.count_nonzero(mask)
                out = tf.layers.conv2d(out, new_filters, 3, padding='same', name=name)
                out = tf.layers.batch_normalization(out, training=True)
                out = tf.layers.max_pooling2d(out, 2, 2, padding='same')
                out = tf.nn.relu(out)
            out = tf.reshape(out, (-1, int(np.prod(out.get_shape()[1:]))))
